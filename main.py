...
"""
    ...
    async def aiimg_generate(
        ...
        if message_id and origin:
           if getattr(self, 'debouncer', None) is not None and self.debouncer.llm_tool_is_duplicate(message_id, origin):
                # 原有逻辑，比如 return 或 log "重复请求"
                logger.info("检测到重复 LLM tool 调用，已跳过")
                return  # 或 continue，根据上下文
